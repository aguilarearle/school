...
class DataPoints:
    ...
    def getStdDeviation(clusters, mean, stddev):
        ...
        for C in clusters:
            ...
            y_std = 0
            for point in C:
                x_std += math.pow(point.x - mean[i][0], 2)
                y_std += math.pow(point.y - mean[i][1], 2)
            x_std = (x_std/N)
            y_std = (y_std/N)
            stddev[i] = [x_std,y_std]
            i += 1
    # -------------------------------------------------------------------
    @staticmethod
    def getCovariance(clusters, mean, stddev, cov):
        ...
        for C in clusters:
            ...
            cover = 0.0
            for point in C:
               cover += (point.x - mean[i][0])*(point.y - mean[i][1])
            cov[i][0] = [stddev[i][0],  cover / N]
            cov[i][1] = [cover / N, stddev[i][1]]
            i += 1
