...
\begin{problem}{}{5}{0}
  ...
  infinite, therfore we also will not weave rugs for lenght $i$ for an infinate
  amount of time.
  ...
  spend all the time along one dimension and $j$ is also countably infinate so
  $2^{i\times j}$ is also countably infinite. Therfore we can weave in one infinate
  day.
  \begin{lstlisting}[mathescape]
    Weaver 2:
  ...
  are countably infinate so the possible patterns for a specific length $i$
  is also countably infinte. Since we are performing triangle scheduling then
  ...
  the weavers dimension and perform triangle scheduling.
  \begin{lstlisting}[mathescape]
...
\begin{problem}{Extra Credit}{6}{0}
  ...
  \begin{align}
    w_{Diag} &= 0.(x_{11}-1)(x_{22}-1)(x_{33}-1)(x_{44}-1)(x_{55}-1)  
  \end{align}
  ...
  \end{align}
  On a specific day we weave a rug wuth a length and width dictated by the
  odd and even intergers in the real represention of the day. 
  ...
  %Map a day to a length and a width i.e day 0.34234234234
  %to length = 0.324324 , width = 0.4324
  %Take the cieling of these and trim. color ^(ceiling[i] \time ceiling[j])
