...
\begin{problem}{}{2}{0}
  ...
  \begin{proof} $ $\newline
    ($\Leftarrow$) Let $w=w_1w_2\dots w_m \in L$. Machine M starts at $q_0$ and
    $\forall \ i \ge 0$ ,$\ q_{i+1} = \delta(q_i, w_i)$. When we reach $w_n$,
    $\delta(q_{n-1}, w_n) \in F$.  \\ \\
    ($\Rightarrow$) Let $w^R=w_nw_{n-1}\dots w_1 \in L_R$. By construction 
    ...
    $\delta(q_1, w_1) \in F^{'} = q_0$, the start state of M.
  \end{proof}
\end{problem}
%-------------------------------------------------------------------------------
% Problem 3.0
...
\begin{problem}{}{3}{0}
  ...
  \begin{proof} $ $\\
    ...
    $\forall \ i \ge 0$ ,$\ q_{i+1} = \delta(q_i, w_i)$. When we reach $w_n$,
    $\delta(q_{n-1}, w_n) \in F$  \\ \\
    ($\Rightarrow$) Let $w_{alt}=w_1w_3w_5\dots w_m \in L_{alt}$. By construction machine
    $M^{'}$ starts at $q_0$. Also by construction of $\delta^{'}(q_i, a \in \Sigma)$
    we skip every other state into a state $q_j$ s.t
    $\delta(q_{i+1}, b \in \Sigma) = q_j$. When we process $w_m$ we will be in one
    of the accepting states $q_{n-2}$ when $m$ is odd and $q_n$ when $m$ is even.
  \end{proof}
\end{problem}
%-------------------------------------------------------------------------------
% Problem 4.0
...
\begin{problem}{}{4}{0}
  We $M = (Q,\Sigma, \delta, q_0, F)$ be a DFA that accepts L. We construct an
  NFA $M^{'}$ that accepts $L_{\frac{1}{2}-}$ \\ \\
  Constructing $M^{'}:$ \\
  1. $Q^{'} = Q \times Q$ \\
  2. $\Sigma = \Sigma$ \\
  3. $\delta^{'}(q_i, a \in \Sigma) =$
  $ \begin{cases} \delta(q_i,a)\\ q_j \ | \ \exists \ \delta(q_j, b \in \Sigma) = q_i \end{cases} $ \\
  4. $q_0^{'} = \{(q_0,q_i) \ | \ q_i \in F \}$ \\
  5. $F^{'} = \{ (q_i, q_j) \ | \ i = j\}$ \\ \\
  Claim: $w$ is accepted by M $\iff w_{\frac{1}{2}-}$ is accepted by $M^{'}$
  \begin{proof}
    ($\Leftarrow$) Let $w=w_1w_2\dots w_m \in L$. Machine M starts at $q_0$ and
    $\forall \ i \ge 0$ ,$\ q_{i+1} = \delta(q_i, w_i)$. When we reach $w_n$,
    $\delta(q_{n-1}, w_n) \in F$  \\ \\
    ($\Rightarrow$) Let $w_{\frac{1}{2}-}=w_1w_2\dots w_{m/2} \ in L_{\frac{1}{2}-}$. By
    construction machine $M^{'}$ starts at $\{ (q_0,q_i) \ | \ q_i \in F \}$. So we
    run two machines the original machine M and a modified version of M in
    reverse. By construction of $\delta^{'}$
  \end{proof}
\end{problem}
\end{document}
